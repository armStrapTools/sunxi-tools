...
{
	uint32_t  jump_instruction;   // one intruction jumping to real code
	uint8_t   magic[8];           // ="eGON.BRM",  not C-style string.
	uint32_t  length;             // generated by PC
	uint8_t   Boot_vsn[4];        // Boot version
	uint8_t   eGON_vsn[4];        // eGON version
	uint8_t   platform[8];        // platform information
...
typedef struct _boot_core_para_t {
    uint32_t user_set_clock;
    uint32_t user_set_core_vol;
    uint32_t vol_threshold;
} boot_core_para_t;

typedef struct _boot1_private_head_t {
    uint32_t prvt_head_size;
    uint8_t prvt_head_vsn[4];
    int32_t uart_port;
    normal_gpio_cfg uart_ctrl[2];
    boot_dram_para_t dram_para;
    ...
    int32_t hold_key_min;
    int32_t hold_key_max;
    uint32_t work_mode;
...
typedef struct _boot_sdcard_info_t {
    int32_t card_ctrl_num;
    int32_t boot_offset;
    int32_t card_no[4];
    int32_t speed_mode[4];
    int32_t line_sel[4];
