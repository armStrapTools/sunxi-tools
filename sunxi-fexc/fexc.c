...
/*
 */
int script_parse(enum script_format format, const char *filename, struct script *script)
{
    int ret = 0;
    switch (format) {
...
int script_generate(enum script_format format, const char *filename, struct script *script)
{
    int ret = 0;
...
int main(int argc, char *argv[])
{
    ...
    if ((script = script_new()) == NULL) {
        perror("malloc");
        exit(EXIT_FAILURE);
    } else if (script_parse(infmt, filename[0], script) && script_generate(outfmt, filename[1], script)) {
